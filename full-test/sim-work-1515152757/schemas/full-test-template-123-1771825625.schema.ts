...
export default defineSchema("full-test-template-123-1771825625", {
  users: defineTable({
  ...
  projects: defineTable({
    ...
    name: c.text().notNull().check("length(name) >= 3"),
    description: c.text(),
  ...
  project_tags: defineTable({
    ...
    added_at: c.text().notNull().default("CURRENT_TIMESTAMP"),
  }),
  ...
  todos: defineTable({
    ...
    title: c.text().notNull().check("length(title) >= 3"),
    description: c.text(),
    status: c.text().notNull().default("todo").check("status in ('todo','in_progress','blocked','done')"),
    priority: c.integer().notNull().default(3).check("priority between 1 and 5"),
