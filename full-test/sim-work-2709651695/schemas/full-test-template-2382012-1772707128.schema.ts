...
export default defineSchema("full-test-template-2382012-1772707128", {
  users: defineTable({
    id: c.integer().primaryKey(),
    email: c.text().notNull().unique().collate("NOCASE"),
    display_name: c.text().notNull().check("length(display_name) >= 5"),
  ...
  projects: defineTable({
    ...
    name: c.text().notNull().check("length(name) >= 6"),
    description: c.text(),
    status: c.text().notNull().default("active").check("status in ('active','archived')"),
    priority: c.integer().notNull().default(3).check("priority between 1 and 5"),
  ...
  todos: defineTable({
    ...
    title: c.text().notNull().check("length(title) >= 5"),
    description: c.text(),
    status: c.text().notNull().default("todo").check("status in ('todo','in_progress','done')"),
    priority: c.integer().notNull().default(3).check("priority between 1 and 5"),
