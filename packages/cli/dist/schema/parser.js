...
export async function loadSchema(filePath) {
    ...
    try {
        ...
        if (schema === undefined) {
            ...
            throw new Error(`No default export in ${fileName}\n\n` +
        ...
        if (typeof schema.name !== "string" || !Array.isArray(schema.tables)) {
            const fileName = basename(filePath);
            throw new Error(`Invalid default export in ${fileName}\n\n` +
        ...
        if (schema.name.trim() === "") {
            throw new Error(`Schema in ${fileName} has no name\n\n` +
                `  The first argument to defineSchema must be a non-empty string:\n\n` +
                `    export default defineSchema("my-app", { ... });\n`);
        }
        // Validate at least one table
        if (schema.tables.length === 0) {
            throw new Error(`Schema "${schema.name}" in ${fileName} has no tables\n\n` +
        ...
        for (const table of schema.tables) {
            if (!table.columns || table.columns.length === 0) {
                throw new Error(`Table "${table.name}" in ${fileName} has no columns\n\n` +
        ...
        if (err instanceof Error && (err.message.includes("No default export") ||
        ...
        if (err instanceof Error) {
            if (err.message.includes("_build is not a function")) {
                throw new Error(`Invalid table definition in ${fileName}\n\n` +
            ...
            if (err.message.includes("is not a function")) {
                throw new Error(`Invalid column definition in ${fileName}\n\n` +
 ...
 */
export async function loadAllSchemas(dir) {
