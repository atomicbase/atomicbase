 ...
 */
async function resolveAmbiguousChanges(changes) {
    const resolutions = [];
    ...
    for (const change of ambiguous) {
        if (change.type === "rename_table") {
            const isRename = await confirm(`  Table '${change.oldName}' was removed and '${change.table}' was added.\n  Is this a rename?`);
            resolutions.push({
                type: "table",
                table: change.table,
            ...
            const isRename = await confirm(`  Column '${change.table}.${change.oldName}' was removed and '${change.table}.${change.column}' was added.\n  Is this a rename?`);
            resolutions.push({
                type: "column",
                table: change.table,
