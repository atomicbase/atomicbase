 ...
 */
function detectAmbiguousChanges(changes) {
    const ambiguous = [];
 ...
 */
async function resolveAmbiguousChanges(changes, ambiguous) {
    const merges = [];
    ...
    for (const change of ambiguous) {
        ...
        if (change.type === "table") {
            const isRename = await confirm(`  Table '${dropChange.table}' was removed and '${addChange.table}' was added.\n  Is this a rename?`);
            if (isRename) {
                merges.push({ old: change.dropIndex, new: change.addIndex });
            }
        }
        else {
            const isRename = await confirm(`  Column '${change.table}.${dropChange.column}' was removed and '${change.table}.${addChange.column}' was added.\n  Is this a rename?`);
            if (isRename) {
                merges.push({ old: change.dropIndex, new: change.addIndex });
            }
        }
 ...
 */
async function pushSingleSchema(api, schema) {
