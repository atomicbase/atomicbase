...
const TEST_TENANT = "sdk-test-tenant";
...
async function runTests() {
  ...
  await tenant.from("sdk_test").delete().where(gt("id", 0));
  ...
  await test("Select all", async () => {
    const { data, error } = await tenant.from("sdk_test").select();
  ...
  await test("Select specific columns", async () => {
    const { data, error } = await tenant.from("sdk_test").select("id", "name");
  ...
  await test("Select with limit", async () => {
    const { data, error } = await tenant.from("sdk_test").select().limit(2);
  ...
  await test("Select count()", async () => {
    const { data, error } = await tenant.from("sdk_test").select().count();
  ...
  await test("Select withCount()", async () => {
    const { data, count, error } = await tenant
      .from("sdk_test")
      .select()
      .limit(2)
      .withCount();
  ...
  await tenant.from("sdk_test").delete().where(gt("id", 0));
  await tenant.from("sdk_test").insert({ id: 1, name: "Alice", email: "alice@example.com", age: 30 });
  await tenant.from("sdk_test").insert({ id: 2, name: "Bob", email: "bob@example.com", age: 25 });
  await tenant.from("sdk_test").insert({ id: 3, name: "Charlie", email: "charlie@example.com", age: 35 });

  // Test basic batch with multiple inserts
  await test("Batch: multiple inserts", async () => {
    const { data, error } = await tenant.batch([
      tenant.from("sdk_test").insert({ id: 10, name: "User10", email: "user10@test.com", age: 20 }),
      tenant.from("sdk_test").insert({ id: 11, name: "User11", email: "user11@test.com", age: 21 }),
      tenant.from("sdk_test").insert({ id: 12, name: "User12", email: "user12@test.com", age: 22 }),
    ]);
  ...
  await test("Batch: mixed insert, update, delete", async () => {
    const { data, error } = await tenant.batch([
      tenant.from("sdk_test").insert({ id: 20, name: "ToUpdate", email: "toupdate@test.com", age: 30 }),
      tenant.from("sdk_test").update({ name: "User10Updated" }).where(eq("id", 10)),
      tenant.from("sdk_test").delete().where(eq("id", 12)),
    ]);
  ...
  await test("Batch: select operations", async () => {
    const { data, error } = await tenant.batch([
      tenant.from("sdk_test").select("id", "name").where(eq("id", 1)),
      tenant.from("sdk_test").select("id", "name").where(eq("id", 2)),
    ]);
  ...
  await test("Batch: single() modifier", async () => {
    const { data, error } = await tenant.batch([
      tenant.from("sdk_test").select().where(eq("id", 1)).single(),
      tenant.from("sdk_test").select().where(eq("id", 2)).single(),
    ]);
  ...
  await test("Batch: single() with no rows returns error object", async () => {
    const { data, error } = await tenant.batch([
      tenant.from("sdk_test").select().where(eq("id", 1)).single(),
      tenant.from("sdk_test").select().where(eq("id", 99999)).single(), // No rows
    ]);
  ...
  await test("Batch: maybeSingle() modifier", async () => {
    const { data, error } = await tenant.batch([
      tenant.from("sdk_test").select().where(eq("id", 1)).maybeSingle(),
      tenant.from("sdk_test").select().where(eq("id", 99999)).maybeSingle(), // No rows - should return null
    ]);
  ...
  await test("Batch: count() modifier", async () => {
    const { data, error } = await tenant.batch([
      tenant.from("sdk_test").select().count(),
      tenant.from("sdk_test").select().where(gt("age", 25)).count(),
    ]);
  ...
  await test("Batch: withCount() modifier", async () => {
    const { data, error } = await tenant.batch([
      tenant.from("sdk_test").select("id", "name").limit(2).withCount(),
    ]);
  ...
  await test("Batch: atomicity - failure rolls back all", async () => {
    // Get current count
    const { data: beforeCount } = await tenant.from("sdk_test").select().count();

    // Try a batch where second operation will fail (insert duplicate id)
    const existingId = 1; // We know this exists
    const { data, error } = await tenant.batch([
      tenant.from("sdk_test").insert({ id: 100, name: "ShouldRollback", email: "rollback@test.com", age: 50 }),
      tenant.from("sdk_test").insert({ id: existingId, name: "Duplicate", email: "dup@test.com", age: 99 }), // Should fail - duplicate id
    ]);
    ...
    const { data: afterCount } = await tenant.from("sdk_test").select().count();
  ...
  await test("Batch: upsert operations", async () => {
    const { data, error } = await tenant.batch([
      tenant.from("sdk_test").upsert({ id: 1, name: "Alice Upserted", email: "alice@example.com", age: 31 }),
      tenant.from("sdk_test").upsert({ id: 200, name: "NewUser", email: "newuser@test.com", age: 40 }),
    ]);
    ...
    const { data: alice } = await tenant.from("sdk_test").select().where(eq("id", 1)).single();
    if (alice?.name !== "Alice Upserted") {
      throw new Error(`Expected 'Alice Upserted', got ${alice?.name}`);
    }

    const { data: newUser } = await tenant.from("sdk_test").select().where(eq("id", 200)).single();
  ...
  await test("Batch: complex mixed operations", async () => {
    const { data, error } = await tenant.batch([
      tenant.from("sdk_test").select().where(eq("id", 1)).single(),           // 0: get single
      tenant.from("sdk_test").select().count(),                                // 1: count all
      tenant.from("sdk_test").insert({ id: 300, name: "BatchTest", email: "batch@test.com", age: 33 }), // 2: insert
      tenant.from("sdk_test").select().where(eq("id", 300)).maybeSingle(),    // 3: verify insert
      tenant.from("sdk_test").update({ age: 34 }).where(eq("id", 300)),       // 4: update
      tenant.from("sdk_test").select("age").where(eq("id", 300)).single(),    // 5: verify update
      tenant.from("sdk_test").delete().where(eq("id", 300)),                  // 6: delete
      tenant.from("sdk_test").select().where(eq("id", 300)).maybeSingle(),    // 7: verify delete
    ]);
  ...
  await tenant.from("posts").delete().where(gt("id", 0));
  await tenant.from("users").delete().where(gt("id", 0));

  // Insert users
  await tenant.from("users").insert({ id: 1, name: "Alice", email: "alice@join.test", age: 30 });
  await tenant.from("users").insert({ id: 2, name: "Bob", email: "bob@join.test", age: 25 });
  await tenant.from("users").insert({ id: 3, name: "Charlie", email: "charlie@join.test", age: 35 }); // No posts

  // Insert posts
  await tenant.from("posts").insert({ id: 1, title: "Alice Post 1", content: "Content 1", user_id: 1 });
  await tenant.from("posts").insert({ id: 2, title: "Alice Post 2", content: "Content 2", user_id: 1 });
  await tenant.from("posts").insert({ id: 3, title: "Bob Post 1", content: "Content 3", user_id: 2 });
